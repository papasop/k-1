...
formula 选择指南:
  'f1' 显式分离  : 类时比例 >0.6 的物理仿真 / 机器人轨迹（硬约束，不可退化）
  'f2' 洛伦兹修正: 兼容旧版（有退化风险，不推荐新项目）
  'f3' 结合版    : 通用（sigma 自适应：类时比例 >0.6 时激活洛伦兹，<0.4 时退化为欧氏）（推荐默认）
...
class LorentzMultiHeadAttention(nn.Module):
    ...
    formula='f3'  结合版，sigma 可学习（推荐默认）
                  适用: 通用（LLM / 视频 / 科学文本 / 物理）
                  特点: sigma 自适应——类时比例 >0.6 时激活洛伦兹，<0.4 时自动退化为欧氏
...
if __name__ == "__main__":
    ...
    print("选择指南:")
    guide = [
        ("f1", "类时比例>0.6的物理/机器人", "硬约束，不退化"),
        ("f2", "加载旧版权重", "有退化风险，不推荐新项目"),
        ("f3", "通用（sigma自适应）", "推荐默认，类时比例>0.6激活洛伦兹，<0.4退化为欧氏"),
    ]
