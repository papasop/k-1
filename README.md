...
| | 传统 LLM / 欧氏模型 | LLCM（洛伦兹） |
|--|---------|------|
| 物理直觉来源 | 语料统计 | 洛伦兹几何内生 |
| 物理守恒保证 | 损失函数惩罚（软约束，可绕过） | 几何上不可达（硬约束）|
| 因果方向 | 对称（正反可互换） | 类时方向单向 |
| 训练方向 | 越训练越反向（ρ=-0.246，实验确认）| 越训练越正确（ρ=+0.259，4.88×放大）|
...
## 为什么当前所有模型都用欧氏几何——以及为什么这是错的

历史上有过用洛伦兹做机器学习的尝试（Lorentzian embeddings、Hyperbolic networks），但都是**假设**某个维度是时间——没有从物理第一原理推导，是盲目的选择。没有时空度规的推导，就无法处理四个核心问题：
...
洛伦兹内积：⟨x,y⟩ = -x_t y_t + x_s y_s  不对称
  时间维度有负号，因果方向单向
```

欧氏模型看"原因→结果"和"结果→原因"完全一样。机器人的动作序列是因果的，欧氏几何在代数上无法编码这个不对称性。洛伦兹的负号是因果方向的代数来源，不是选择，是推导结果。

### 2. 无法编码零代价方向

```
Assumption R：存在方向使位移代价趋向零
  （机器人匀速运动时，沿速度方向不需要额外能量）
...
欧氏模型无法在几何上表示"这个方向不消耗能量"，只能用损失函数惩罚。**惩罚可以被优化器绕过，几何结构不能。** 这就是为什么欧氏模型的物理守恒总是软约束——在训练分布外必然失效。
...
Theorem 4（K=1 Chronogeometrodynamics）：
  ...
  一行证明：dc > 0 ⟺ -1/det G > 0 ⟺ det G < 0
```

欧氏：det G > 0 → dc = 0，系统没有几何上的稳定边界，稳定性只能靠损失函数强制。洛伦兹：det G < 0 → dc > 0，稳定边界从几何上涌现，守恒律不需要损失函数，是几何的必然结果。

**实验确认：** F3（洛伦兹）动量守恒损失比欧氏低10倍（0.025 vs 0.275），即使 sigma 未激活也成立——这是几何结构效应，不是训练信号差异。

### 4. 无法处理信息时间度量

```
dt²_info = Σ_q Φ_q/H_q

这个度量的 Hessian 在可实现性条件（R+E+T）下被强制为不定矩阵
欧氏假设等价于 Fisher 信息矩阵——永远正定
```

Fisher 信息矩阵永远正定 → 没有类时方向 → 没有因果结构。欧氏模型在参数空间里是"纯空间性"的：所有参数方向等价，没有时间轴，无法在参数空间里编码"先发生什么、后发生什么"。这意味着欧氏模型学到的表征在结构上就缺少时间轴——不是能力问题，是代数结构问题。

### 实验验证：欧氏越训练越反向

```
F3 洛伦兹：训练前 ρ=+0.053（随机），训练后 ρ=+0.259（放大4.88×）
欧氏模型：  训练前 ρ=-0.100（随机），训练后 ρ=-0.246（放大2.46×，反向）
...
这不是欧氏模型能力不足的问题。**几何签名决定了训练能学到什么方向。** 欧氏模型在物理代价方向上的反转是数学必然，数据再多也无法修正，因为错误来自几何签名，不来自数据量。
...
### 修正猜想：最优控制对应最深类时（Theorem 6实验支持）

```
猜想：最优控制轨迹（HJB最优解）的洛伦兹嵌入 mq → 0
...
如果成立：
  HJB求解（指数复杂）≡ 找mq=0的路径（多项式）
  实时最优控制 = 一次神经网络前向传播
```

验证代码：`experiments/hjb_lorentz_conjecture.py`
...
- ✅ 两阶段微调（方向A和B解耦训练）
- ✅ 双向推力损失（守恒→类时，非守恒→类空）
- ✅ 修正猜想：欧氏反转实验发现（ρ=+0.259 vs ρ=-0.246，4.88×放大）

**进行中：**

- 🔄 里程碑2：sigma激活（当前0.533，目标>0.60）
- ✅ 里程碑5：修正猜想验证（4/5通过，摆R²=0.9997）
