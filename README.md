...
> **核心命题：F3 洛伦兹 backbone 建立的表示空间里，感知流形自发形成（层2），物理嵌入与语言描述的余弦相似度显著更高（方向A），语言指令生成守恒轨迹（方向B）。**
> 复现脚本：`experiments/layer1_minimal_test.py`（一个文件验证全部三层）

| 指标 | 欧氏 | LLCM F3 | 差异 | p值 | d | seeds |
|------|------|---------|------|-----|---|-------|
| 语言对齐得分 | 0.170±0.023 | **0.297±0.039** | +0.127 | **0.0091** | **2.12** | 5/5 |
| embed_seq mq均值 | +72.5 | **+0.9** | mq差距76.5倍，p=0.0000，d=12.52 | <0.001 | 12.52 | 5/5 |
...
> **层2（mq差距47倍）**：感知流形存在——守恒运动嵌入自发偏向类时区域，无任何语言信号。
> **方向A（p=0.0091）**：感知流形可以被语言索引——物理嵌入与语言描述余弦相似度显著更高。
> **方向B（p=0.0167）**：语言可以激活物理本能——语言指令生成守恒轨迹，无物理损失函数。
> 三层同时成立 → 婴儿说话静态闭环完整。里程碑编号见下方婴儿说话项目里程碑节。
...
### 里程碑0：感知流形存在，且分离先于语言（层2验证） ✅

```
实验：experiments/layer1_minimal_test.py（embed_seq层2测量）
结论：F3 mq均值=+0.948 vs 欧氏=+72.510，差距76.5倍，5/5 seed
      p=0.0000  d=12.52（所有指标中效应量最大）
      逐标签分离：stable嵌入偏类时，changing嵌入偏类空
      纯物理预训练阶段测量，无任何语言信号参与

关键顺序：
  1. 物理预训练（无语言）→ 感知分离自发形成
  2. 语言对齐（方向A）  → 在已有分离上贴标签
  3. 语言激活（方向B）  → 标签反向激活对应的感知区域

含义：守恒/非守恒运动的感知分离不是语言教会的
      是 F3 几何结构在物理经验中自发产生的
      语言找到了已经存在的分离，贴上了符号
      这是婴儿说话机制最深的一层：
      先有感知分离，后有语言贴标签
...
含义：F3 的光锥几何充分激活后，
      ...
      类时/类空方向真正分离，对齐得分预期进一步提升
注意：里程碑2不是里程碑3的前提——静态闭环已经完整
      里程碑2是优化方向，让结果更强，不是阻断条件
```
...
```
里程碑0 ✅ → 感知流形存在，mq差距47倍（最震撼的结果）
里程碑1 ✅ → 方向A成立，p=0.0091，d=2.12
里程碑3 ✅ → 方向B成立，p=0.0167，d=1.77
────────────────────────────────────────
里程碑0+1+3 同时成立 → 婴儿说话静态闭环完整

里程碑2 ⬜ → sigma > 0.60（频域升级前提）
里程碑4 ⬜ → 动态交互：Law II 在线学习验证
```

> 里程碑0/1/3 完成了静态闭环——感知流形存在，语言可以索引，语言可以激活。
> 里程碑4（动态交互）是婴儿说话从静态结构到动态学习的下一步。
...
- ✅ 层2感知流形分离：mq差距76.5倍，p=0.0000，d=12.52（5/5 seed，效应量最大）【里程碑0】
...
- 📋 动态交互验证（Law II 在线学习）
      婴儿感知流形在实时交互中精化
      Law II：dx/dt = (JG−D)∇V_total，语言反馈驱动在线更新
      验证脚本：experiments/online_interaction_test.py
- 📋 GPT-2 规模验证（768维/12层）
- 📋 D4RL 机器人数据集
- 📋 v1.1.0 PyPI 发布
