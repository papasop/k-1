...
(function(){
  ...
  function mount(){
    ...
    function update(){
      ...
      // f = 1 - 2M/r - Λr²/3.  In KKernel: C₁ = 2M, C₃ = -Λ/3.
      ...
      const T1 = 1 - 2*L*r*r;     // K₁ target
      const T2 = 1 - L*r*r;        // K₂ target
      const dK1 = K1 - T1;
      const dK2 = K2 - T2;

      // Forward: residual ‖R_μν − Λ g_μν‖
      const E = KK.Ricci(r, fv, fp(r), fpp(r));
      // g = diag(-f, 1/f, r², r² sin²θ) → check Rtt+Λf, Rrr−Λ/f, Rθθ−Λr²
      ...
      if (!okDomain){
        ...
        v.innerHTML = `<b>EXTENSION HOLDS.</b> Both K-targets met (Δ ~ ${dK1.toExponential(1)}, ${dK2.toExponential(1)}) ` +
                      `and ‖R_μν − Λg_μν‖ = ${maxR.toExponential(1)}. The Λ-Einstein extension of Theorem 5.2 is verified at this point on SdS.`;
      } else if (Rok && !Kok){
        v.className='ac-verdict verd-fail';
