...
(function(){
  ...
  const TOL = 1e-9;  // numerical tolerance for "equality" claims
  ...
  function setResid(el, pairs, tol){
    // pairs: [[label, value], ...]; tol shown at end
  ...
  function verdict(el, pass, text){
    el.textContent = text;
    if (pass === null){
      // neutral / informational — no pass-or-fail claim made
  ...
  // ---------- Schwarzschild & general static SS helpers ----------
  function mk_f(C1, C2){
  ...
  function mountRow2(){
    ...
    function drawP(){
      const kl = +kel.value; kelv.textContent = kl.toFixed(2);
      // base point: x_* = (0, 1)  (the K=1 G-eigenvector with eigenvalue μ = 1)
      // K(x_*) = -(κℓ)²·0² + 1·1² = 1   — independent of κℓ by construction
      // x_*ᵀ G² x_* = (κℓ)⁴·0 + 1·1 = 1 — also independent of κℓ at this base point
      // What κℓ does control: the *other* eigenvalue μ₁ = −(κℓ)², which sets the
      // hyperbolic curvature of the K=1 locus away from x_*.
      const x1 = 0, x2 = 1;
      const K  = -(kl*kl)*x1*x1 + x2*x2;       // = 1
      const xGGx = (kl*kl)*(kl*kl)*x1*x1 + x2*x2; // = 1
      const μ_other = -(kl*kl);                 // depends on κℓ
      Kout.textContent = fmt(K, 6);
  ...
  function mountRow3(){
    ...
    function drawP(){
      ...
      const K = -x1*x1 + 1;  // at (x1, 1) with κℓ=1
      const V = 0.5*(K-1)*(K-1);
    ...
    function drawF(){
      ...
      const condR = rR  < TOL;
      // Honest framing: this single point on a single ansatz never proves the
      // equivalence (K₁=K₂=1 ⇔ Rμν=0). It only checks that the two sides agree
      // *at this sample*. A real proof scan lives in §P / proofmap.
      // On the f = 1 − C₁/r − C₂/r² family in particular, K₁ ≡ 1 identically
      // (algebraic — see paper §5), so this row's slider can never produce a
      // R=0 / K≠1 counter-example: the failure mode is filtered out by the
      // ansatz itself. We say so out loud rather than claim "verified".
      if (condK && condR){
  ...
  function mountRow4(){
    ...
    function drawP(){
      ...
      const r = Math.abs(K-1);
      setResid(residP, [['|K−1|', r], ['V', V]], TOL);
      // Schematic / illustrative — V = ½(K−1)² is just the potential. There is
      // no theorem-vs-numerics test in the point column here; we only display
      // the value at the user's slider position. We say so out loud.
      if (r < 1e-6)
    ...
    function drawF(){
      ...
      // This residual is a definitional round-trip, not a test of the theorem:
      // K₂ ≡ 1 − 8πρr² *defines* the matter→K₂ map, and ρ_rec inverts it. The
      // residual must be at machine epsilon by construction. We label it as such.
      const ok = resid < TOL;
      verdict(vF, ok, ok
  ...
  // ============================================================
  // Paper statement toggles (collapse/expand the quoted rows)
  // ============================================================
